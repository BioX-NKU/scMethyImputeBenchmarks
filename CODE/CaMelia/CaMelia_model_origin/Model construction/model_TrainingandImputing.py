...
def f1(m, X_train, X_test):
    ...
    b = m.predict_proba(X_test)[:, 1]
    b = np.int64(b >= 0.5)
    return (f1_score(y_train, a), f1_score(y_test, b))
...
if __name__ == '__main__':
    ...
    for i in range(len(filenames_out)):          
        ...
        for train_index, test_index in kf.split(X, Y):
            ...
            kf_F1.append(f1(model, X_train, X_test)[1])
        ...
        F1.append(round(np.mean(kf_F1), 4))
    ...
    data['MCC'] = MCC
    data['F1'] = F1
    data.to_csv(r'%s/5fold_crossvalidation_catboost1.csv' % gse, sep=',', header=True, index=False)